...
toOut = @(x) (nOut==1) * x(:) + (nOut==2) * [x(:) x(:)];
writeF = @(frame) writer(frame);  % frame must already be Nx nOut
...
dur  = 0.20;  % seconds, beep duration
N = floor(dur * fs / F) * F;
t = (0:N-1)'/fs;
...
for itrial = 1:cfg.ntrials
    ...
    if ~trials.catch_trial(itrial)
        ...
        while (now_secs() - trialStart) < cfg.text_stim_dur && ...
            ...
            tStart  = now_secs();
            audioIn = reader();
            delayed = vfd(audioIn, cfg.audio_sample_rate * trials.delay(itrial) / 1000);
        ...
        % Tail flush
        for iframe = 1:ceil((max(cfg.delay_values_ms)/1000) * cfg.audio_sample_rate / cfg.audio_frame_size) + 5
            audioOut = vfd(zeros(cfg.audio_frame_size,1), cfg.audio_sample_rate * trials.delay(itrial) / 1000);

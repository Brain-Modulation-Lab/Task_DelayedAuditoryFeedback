...
tone = 0.2 * (tone .* env);   % safe amplitude

% Helper: stream vector in frame-sized chunks via audio_writer
...
for i = 1:3
    ...
    % optional: brief photodiode flash during each beep
    set(hSquare,'FaceColor',[0 0 0]);  % black
    playChunked(tone);
    set(hSquare,'FaceColor',[1 1 1]);  % white (baseline)
    pause(gap);
end

% Flush any residual audio in writer/VFD with SILENCE (no mic passthrough)
silentFrame = zeros(cfg.audio_frame_size, info(writer).MaximumOutputChannels);
setup(writer, silentFrame);
for k = 1:10
    writer(silentFrame);
    vfd(zeros(cfg.audio_frame_size,1), 0);
end
...
for itrial = 1:cfg.ntrials
    ...
    if ~trials.catch_trial(itrial)
        ...
        while (now_secs() - trialStart) < cfg.text_stim_dur && ...
            ...
            if size(audioOut, 2) == 1 && info(writer).MaximumOutputChannels >= 2
                audioOut = repmat(audioOut, 1, 2); % duplicate mono to stereo
            end
            writer(audioOut);
        ...
        silentFrame = zeros(cfg.audio_frame_size, info(writer).MaximumOutputChannels);
        for iframe = 1:maxDelayFrames
            audioOut = vfd(silentFrame(:,1), delay_samples); % use mono input for fractional delay
            if size(audioOut, 2) == 1 && info(writer).MaximumOutputChannels >= 2
                audioOut = repmat(audioOut, 1, 2); % duplicate mono to stereo
            end
            writer(audioOut);

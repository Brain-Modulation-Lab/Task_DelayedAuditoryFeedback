...
% Wait until SPACE is pressed on 'fig'. Sets appdata 'escPressed' on ESC.
    setappdata(fig,'__space',false);
    oldCb = get(fig,'WindowKeyPressFcn');
    set(fig,'WindowKeyPressFcn', @(~,evt) onKey(fig, evt));
    while ~getappdata(fig,'__space') && ~getappdata(fig,'escPressed')
        drawnow; pause(0.01);
    end
    set(fig,'WindowKeyPressFcn', oldCb);
    if isappdata(fig,'__space'), rmappdata(fig,'__space'); end
end
...
function onKey(fig, evt)
    try
        if isfield(evt,'Key')
            ...
            if strcmp(evt.Key,'space'),  setappdata(fig,'__space',   true); end
        end
        % Some MATLAB versions populate Character instead
        if isfield(evt,'Character') && isequal(evt.Character,' ')
            setappdata(fig,'__space', true);
        end
    catch

...
% Wait until SPACE is pressed on 'fig'. Sets appdata 'escPressed' on ESC.
    setappdata(fig,'spacePressed', false);   % <-- valid name
    oldCb = get(fig,'WindowKeyPressFcn');
    set(fig,'WindowKeyPressFcn', @(~,evt) onKey(fig, evt));
    while ~getappdata(fig,'spacePressed') && ~getappdata(fig,'escPressed')
        drawnow; pause(0.01);
    end
    set(fig,'WindowKeyPressFcn', oldCb);
    if isappdata(fig,'spacePressed'), rmappdata(fig,'spacePressed'); end
end
...
function onKey(fig, evt)
    try
        if isfield(evt,'Key')
            if strcmp(evt.Key,'escape'), setappdata(fig,'escPressed',  true); end
            if strcmp(evt.Key,'space'),  setappdata(fig,'spacePressed', true); end
        end
        % Some MATLAB versions populate Character as the space
        if isfield(evt,'Character') && isequal(evt.Character,' ')
            setappdata(fig,'spacePressed', true);
        end
    catch
        % swallow any spurious callback errors
    end
end

...
% START_DAF_INTRAOP_MASTER - Intraoperative launcher for the Delayed Auditory Feedback (DAF) task
%
% This function configures and launches the DAF master task, which controls
% the experiment presentation, timing, and communication with the DAF engine.
...
cfg.LagMetrics    = false;          % Whether to enable lag metrics (overridden by engine)
...
cfg.TASK_FUNCTION = 'task_daf_master.m';% Task main function filename

% DAF control (USB serial to DAF computer)
cfg.DAF_CONTROL        = 'usb'; % Communication mode (usb)
cfg.USB_PORT_MASTER   = "COM7"; % USB port for DAF engine control (change as needed)
cfg.USB_BAUDRATE      = 115200; % Baud rate for serial communication
cfg.DAF_ACK_TIMEOUT_S = 0.100;  % Timeout for DAF serial ack (seconds)
cfg.DAF_MAX_RETRIES   = 2;      % Number of retries for sending commands
cfg.DAF_START_OFFSET_S = 0.000; % Start offset time for DAF engine relative to stimulus (seconds)
...
% Ensure diary is turned off when function exits
...
try
    ...
    configureTerminator(s,"LF"); % Use LF as line terminator
    cfg.DAF_SERIAL = s;
...
sCleanup = onCleanup(@() tryCloseSerial(cfg)); % Ensure serial port is properly closed on function exit or error
...
%% Helper function to safely close the USB serial port
function tryCloseSerial(cfg)
try
    if isfield(cfg,'DAF_SERIAL') && ~isempty(cfg.DAF_SERIAL) && isvalid(cfg.DAF_SERIAL)
        if isa(cfg.DAF_SERIAL,'serialport')
            delete(cfg.DAF_SERIAL);  % correct for serialport
        else
            fclose(cfg.DAF_SERIAL);  % legacy serial()
            delete(cfg.DAF_SERIAL);
        end
    end
catch
end

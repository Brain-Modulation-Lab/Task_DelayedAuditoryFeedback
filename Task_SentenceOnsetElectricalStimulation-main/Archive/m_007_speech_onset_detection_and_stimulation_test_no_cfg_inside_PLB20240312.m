...
% function m_007_speech_onset_detection_and_stimulation_test_no_cfg_inside(cfg)
fprintf('Initializing Sound...\n'); 
PsychDefaultSetup(2);
InitializePsychSound; 
% wait until voice detected, then continue
recording_handle=cfg.RecorderHandle;
detection_ops=cfg.DetectionOPS;
% audio_in_t = PsychPortAudio('GetAudioData', recording_handle);

speech_onset_detection(recording_handle, detection_ops); 
...
% subject and session
cfg.SUBJECT = 'testAB1212' ; %subject identifier
...
PsychPortAudio('Start', recording_handle);
[audio] = PsychPortAudio('GetAudioData', recording_handle);
